...
def generate_X_Y(text):
    ...
    while (iteration + 2) * time_steps < len(text):
        ...
        for ch in input[1:]:
            y.append(one_hot_encodings(ch))
        y.append(one_hot_encodings(' '))
...
rnn = keras.layers.SimpleRNN(units=64,
                             ...
                             return_sequences=True,  # yhat at every iteration
...
history = model.fit(x=np.array(X), y=np.array(Y), epochs=10)
